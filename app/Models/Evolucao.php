...
class Evolucao
{
    ...
    public static function save($evolucao, $agendamento_id)
    {
        ...
        if (empty($evolucao && $agendamento_id)) {
        ...
        // insere no banco
        $DB = new DB;
        $sql = "INSERT INTO evolucao(agendamento_id, evolucao) VALUES(:agendamento_id, :evolucao)";

        $stmt = $DB->prepare($sql);
        $stmt->bindParam(':agendamento_id', $agendamento_id);
        $stmt->bindParam(':evolucao', $evolucao['evolucao']);
